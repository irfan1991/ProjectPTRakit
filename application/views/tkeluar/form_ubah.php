    ...
    <link href="<?php echo base_url();?>media/css/bootstrap-datepicker.min.css" rel="stylesheet" type="text/css" />
      ...
      <div class="content-wrapper">
        ...
        <section class="content">
			...
			<input type="hidden" id="hidden" name="did" value="<?= $sqlH['id_trx_hdr'];?>">
				<input type="hidden" name="user_c" value="<?php echo $username;  ?>">
				<div class="form-group">
					<label for="username">No Trx Keluar</label>
					<input id="notrx" name="notrx" class="form-control" type="text" placeholder="Enter NO TRX" data-rule-required="true" readonly="true" value="<?= $sqlH['no_trx_hdr'];?>">
				</div>
				<div class="form-group">
					<label>Tanggal</label>
				...
				<div class="form-group">
					...
					<input class="form-control" type="text" name="amnanesa" id="amnanesa" placeholder="Max 100 Karakter" data-rule-required="true" value="<?= $sqlH['amnanesa'];?>">
				</div>
				<div class="form-group">
					<label>Diagnosa</label>
					<input class="form-control" type="text" name="diagnosa" id="diagnosa" placeholder="Max 100 Karakter" data-rule-required="true" value="<?= $sqlH['diagnosa'];?>">
				</div>
				<div class="form-group">
				    <label>Nama Karyawan</label>
				    <?php
						
						echo form_dropdown('kary',$kary,$sqlH['id_karyawan'],'id="" class="form-control chosen" data-placeholder="-- Pilih Karyawan --"');
				    ?>
				</div>
					...
					<div class="form-group">
							...
							<?php	if($sqlH['status'] == 0){ ?>
						<option value ="0" selected>Kecelakaan </option>
       				  <option value="1">Non Kecelakaan</option>
							<?php	}else{ ?>
						 <option value="1" selected=>Non Kecelakaan</option>
...
if (!empty($sqlD) ){
	...
	foreach($sqlD as $r):
		...
		echo "<td><input type='text' name='qtyold[]' class='form-control qty'  readonly='true' id='qty".$i."' data-rule-required='true' data-rule-number='true'value='".$r->qty."' /></td>";
		echo "<td>".$r->satuan."</td>";
		echo "<td><input type='text' name='hargaold[]' class='form-control harga' readonly='true' id='harga".$i."' data-rule-required='true' data-rule-number='true' value='".$r->harga."' /></td>";
		$ttl = $r->qty * $r->harga;

		echo "<td><input type='text' name='totalold[]' class='form-control total' ireadonly='true' id='total".$i."data-rule-required='true' data-rule-number='true' readonly='true' value='".$ttl."'/></td>";
		?>
		 <td> 
		<a href="<?php echo site_url('transaksiK/ubahtransaksi/'.$r->id_trx_dtl); ?>" class="tip btn btn-sm btn-default"  title="Edit" ><i class="glyphicon glyphicon-edit" ></i></a> 
      
    		&nbsp | &nbsp
		 <a href="#" data-url="<?php echo site_url('transaksiK/hapustransaksi/'.$r->id_trx_dtl); ?>" class="tip btn btn-sm btn-default confirm_delete"  title="Cetak" ><i class="glyphicon glyphicon-trash" ></i></a><td> 
		  <?php
		echo "</tr>";
		$total += $ttl;
		$i++; 
...
$(document).ready(function() {
	...
	function tambah()
	{
		...
		if (emptyrows == 0 ) {
			...
			var txtHrg = '<input type="text" name="harga[]" class="form-control harga" id="harga'+lastRow+'" data-rule-required="true" data-rule-number="true" readonly="true" />';
