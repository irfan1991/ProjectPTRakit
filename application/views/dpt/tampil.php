          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                <div class="box-body">
				  <p>
					  <a href="<?= site_url();?>departement/tambah" class="btn btn-primary">Tambah</a>
...
<td><?php echo $r->nama; ?></td>
       <td>
     <a href="<?php echo site_url('departement/ubah/'.$r->id); ?>" class="tip btn btn-sm btn-default"  title="Edit" ><i class="glyphicon glyphicon-edit" ></i></a> 
      
    &nbsp | &nbsp 
   <a href="#" data-url="<?php echo site_url('departement/hapus/'.$r->id); ?>" class="tip btn btn-sm btn-default confirm_delete"  title="Hapus" ><i class="glyphicon glyphicon-trash" ></i></a> 
      </td>
...
<?php $this->load->view('layout/jscript');?>
  ...
  $('.confirm_delete').on('click', function(){
    ...
    return false;
      ...
      {
		  window.location.href = "<?= site_url();?>/departement/ubah/"+x;
	  }
      ...
      function deleteAction(x)
      {
		BootstrapDialog.confirm('<center><h4>Yakin ingin hapus data ini ?</h4></center>', function(result){
            if(result) {
				$.post('<?= site_url();?>/departement/hapus',{id : x} ,
					function(e)
					{
						var r = e.split('#');
						if(r[0] == 'S')
